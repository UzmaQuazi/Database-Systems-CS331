...
# def run_queries_as_transaction(queries, query_db, assignment):
...
#             query_text = query[0]
#             query_desc = "N/A" if len(query) < 2 else query[1]
#             query_values = None if len(query) < 3 else query[2]
...
#             log_query(query_text, query_desc, query_db, len(rows), user, assignment, duration, conn)
#             conn.close()
...
#                 results.append([headers, data, cursor.description])
#         conn.commit()
#     except:
#         conn.rollback()
