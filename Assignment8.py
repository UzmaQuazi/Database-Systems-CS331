...
# 2[2b] Create a Python function to_xml(headers, data) that converts the headers and data into XML format
def xml_clean(item):
    return str(item).replace("&", "&amp;")

def to_xml(name, headers, data):
    nl = "\n"
    headers = [header.replace(" ", "") for header in headers]
    x_header = '<?xml version="1.0" encoding="UTF-8"' + '?>'
    x_title = nl + ou.create_element("title", xml_clean(name))
    content = ""
    for row in data:
...
# 3[2a] Create a Python function to_csv(headers, data) that converts the headers and data into CSV format
def to_csv(headers, data):
    s_headers = ",".join(headers)
    s_data = "\n".join([",".join([str(col) for col in row]) for row in data])
    return s_headers + "\n" + s_data

# 1[2c] Create a Python function to_json(headers, data) that converts the headers and data into JSON format
...
# [3a] Create a Python function from_xml(xml) that converts the xml into headers (1D) and data (2D)
def from_xml(xml):
    print(xml)
    headers = []
    data = []
    ...
    while idx_row > 0:
        ...
        for element in elements:
            ...
            if len(headers) < len(elements):
                header = elements[1: idx_begin_content-1]
...
# [3c] Create a Python function from_json(json) that converts the json into headers (1D) and data (2D)
    # Teitelman said leave this for a rainy day / Or do for Brownie points
